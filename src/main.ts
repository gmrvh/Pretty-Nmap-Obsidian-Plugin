...
class NmapFormatterPlugin extends Plugin {
    async onload() {
        ...
        this.addCommand({
            ...
            editorCallback: (editor) => {
                ...
                if (!selection || selection.trim() === "") {
                    new Notice("No text selected to format.");
                    return;
                }
    ...
    formatNmapOutput(rawOutput: string): string {
        ...
        for (const line of lines) {
            if (line.match(/^\d+\/tcp\s+(open|closed|filtered)/)) {
                ...
                const formattedLine = line.replace(
                    /(\d+\/tcp\s+)(\w+)/,
                    (_, port, state) =>
                        `- **${port}<span style="color:${this.getStateColor(state)}">${state}</span>**`
                );
                isNmapOutput = true;
...
export default NmapFormatterPlugin;
